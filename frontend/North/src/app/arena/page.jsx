...
export default function ArenaPage() {
  ...
  useEffect(() => {
    // Fetch problems and quizzes in parallel, then auto-open the first available level
    (async () => {
      setLoading(true);
      try {
        ...
        // after both fetched, auto-open the first level that has content
        const p = {
          beginner: (problemsRes || []).filter(
            (pr) => pr.difficulty === "beginner"
          ).length,
          intermediate: (problemsRes || []).filter(
            (pr) => pr.difficulty === "intermediate"
          ).length,
          advanced: (problemsRes || []).filter(
            (pr) => pr.difficulty === "advanced"
          ).length,
        };
        const q = {
          beginner: (quizzesRes || []).filter(
            (qu) => qu.difficulty === "beginner"
          ).length,
          intermediate: (quizzesRes || []).filter(
            (qu) => qu.difficulty === "intermediate"
          ).length,
          advanced: (quizzesRes || []).filter(
            (qu) => qu.difficulty === "advanced"
          ).length,
        };

        const firstAvailable =
          ["beginner", "intermediate", "advanced"].find((l) => p[l] || q[l]) ||
          "beginner";

        if (firstAvailable) setSelectedLevel(firstAvailable);
      } catch (err) {
        console.error("Error initializing Arena:", err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const fetchProblems = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/code/problems?userId=${userId}`
      );
  ...
  const fetchQuizzes = async () => {
    try {
      const data = await getQuizzes(); // getQuizzes should return an array or []
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-[#F2F2F2] font-mono selection:bg-[#CCFF00] selection:text-black flex flex-col">
        ...
        <div className="col-span-12 md:col-span-8 lg:col-span-9 p-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-[#0a0a0a]">
          <div className="max-w-xl">
            ...
            <p className="text-sm text-[#B8B8B8] leading-relaxed uppercase">
              Engage in tactical coding challenges. Select your clearance level
              to access classified protocols. Efficiency is mandatory.
            </p>
          </div>
          <a
            href="/"
          ...
          >
            <span className="text-[#CCFF00] group-hover:-translate-x-1 transition-transform">
              ←
            </span>
            RTB_BASE
          </a>
      ...
      <div className="flex-1 max-w-[1920px] mx-auto w-full grid grid-cols-12">
        ...
        {!loading && (
          <>
            {Object.entries(levels).map(([difficulty, problemsList]) => {
              ...
              const quizzesForLevel = quizLevels[difficulty] || [];
              //   begginers ke liye ye unlock rahe ga
              const isLocked =
                difficulty !== "beginner" &&
              ...
              return (
                <div
                  ...
                  onClick={() =>
                    !isLocked &&
                    setSelectedLevel(isSelected ? null : difficulty)
                  }
                >
                  {/* Active Marker */}
                  {isSelected && (
                  ...
                  <div className="p-8 md:p-12 border-b border-[#333333] flex-1 relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start mb-12 relative z-10">
                      ...
                      <span className="text-[10px] text-[#555555] uppercase">
                        {problemsList.length + quizzesForLevel.length} PROTOCOLS
                      </span>
                    </div>
                  ...
                  {isLocked && (
                    <div className="absolute inset-0 bg-[#000]/80 backdrop-blur-[2px] flex items-center justify-center border border-[#333333]">
                      <div className="border border-[#333333] bg-[#0a0a0a] px-6 py-4 text-center">
                        <span className="block text-2xl mb-2 text-[#333333]">
                          🔒
                        </span>
